...
rainbow_colors <- palette(c("#494947", # 1
                            "#7494EA", # 2
                            "#B14AED", # 3
                            "#44CCFF", # 4
                            "#35FF69", # 5
...
cotas <- function(dimension){
  ...
  p = qplot(xlab=expression(italic(H)), ylab=expression(italic(C))) +
    ...
    theme_few(base_size = 18, base_family = "sans")  + 
...
pHC <- HCPlane(pHC, Entropy.Complexity.periodic, dimension,0) + theme_minimal(base_size = 25, base_family = "sans")

p0 <- qplot(x=subx, y=series_fk[subx,1], geom="line", color=I(rainbow_colors[1]), xlab="", ylab="") +
  ggtitle("White Noise") + 
  theme(plot.title = element_text(hjust=0.5)) + theme_tufte(base_size = 15, base_family = "sans")
...
legend.names = c("White Noise", bquote(f^-1/2), bquote(f^-1))
...
p = p +
  ...
  xlim(limits = c(0.985, 1)) + ylim(limits = c(0, 0.05)) + theme_minimal(base_size = 25, base_family = "sans") +

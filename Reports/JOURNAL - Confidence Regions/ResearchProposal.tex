...
Denote this situation $X\sim\text{dbd}(\alpha,\beta)$.

This model can also be related to the Beta distribution with parameters $\alpha$ and $\beta$.
...
Since we do not know the distribution of $(\mathfrak{H},\mathfrak{C})$, we will estimate~\eqref{Eq:Problem}.
...
Now we assume that $\bm h_i$ is an outcome of size $N$ of $X\sim\text{dbd}(\alpha,\beta)$, and we compute $\widehat\alpha_i,\widehat\beta_i$.
...
We may then use $\text{dbd}(\overline{\widehat{\alpha}_i},\overline{\widehat{\beta}_i})$ distribution as a model for the histograms.
From there, we may obtain an estimate of the distribution of $(\mathfrak{H},\mathfrak{C})$.

The problem was knowing $\Pr\big((\mathfrak{H},\mathfrak{C})\in S\big)$, but $\mathfrak{H}=\mathfrak{H}(X)$  and $\mathfrak{C} = \mathfrak{C}(X)$, and we may assume that $X\sim\text{dbd}(\alpha,\beta)$, and we may estimate $(\alpha,\beta)$.
The problem becomes finding the distribution of $\big(\mathfrak{H}(X),\mathfrak{C}(X)\big)$ under $X\sim\text{dbd}(\alpha,\beta)$.

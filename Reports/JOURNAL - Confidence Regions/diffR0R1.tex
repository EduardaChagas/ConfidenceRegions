...
%DIF DEL IT-ConfidenceRegions.tex     Fri Oct 29 07:11:47 2021
%DIF ADD IT-ConfidenceRegionsR1.tex   Fri Oct 29 17:21:51 2021
...
%DIF 26c27
%DIF < \title{A Test for White Noise in the Entropy-Complexity Plane}
%DIF -------
\title{Time Series Analysis with Bandt \& Pompe Symbolization and a Test for White Noise in the Entropy-Complexity Plane} %DIF > 
%DIF -------
...
%DIF 34c35
%DIF < \author[2]{Cristopher G.\ S. Freitas}
%DIF -------
\author[2]{\mbox{Cristopher G.\ S. Freitas}} %DIF > 
%DIF -------
...
\begin{abstract}
	\DIFaddbegin \DIFadd{This article serves two purposes.
	Firstly, it surveys the \mbox{%DIFAUXCMD
\citeauthor{PermutationEntropyBandtPompe} }\hspace{0pt}%DIFAUXCMD
methodology for the statistical community, stressing topics that are open for research.
	...
	%DIF > 
	}\DIFaddend The \citeauthor{PermutationEntropyBandtPompe} methodology 
	\DIFdelbegin \DIFdel{has been used successfully in the analysis of time series. It }\DIFdelend %DIF > has been used successfully in the analysis of time series. 
	%DIF > It 
	...
	This article \DIFdelbegin \DIFdel{proposes }\DIFdelend \DIFaddbegin \DIFadd{provides }\DIFaddend the first proposal
	of a test in the \DIFdelbegin \DIFdel{entropy-complexity }\DIFdelend \DIFaddbegin \DIFadd{Entropy-Complexity }\DIFaddend plane for the
	white noise hypothesis.
	Our test is based on true white noise sequences obtained from physical devices.
	The proposed methodology provides consistent results\DIFdelbegin \DIFdel{, 
			managing to assess }\DIFdelend \DIFaddbegin \DIFadd{: 
	it assesses }\DIFaddend sequences of true random samples as random (adequate \DIFaddbegin \DIFadd{test }\DIFaddend size), 
	\DIFdelbegin \DIFdel{rejecting correlated }\DIFdelend \DIFaddbegin \DIFadd{rejects correlated and contaminated }\DIFaddend sequences (sound \DIFaddbegin \DIFadd{test }\DIFaddend power), and
	\DIFdelbegin \DIFdel{capturing }\DIFdelend \DIFaddbegin \DIFadd{captures }\DIFaddend the randomness of generators previously analyzed in the literature.
...
The resulting distribution is less sensitive to outliers than the original data, and the histogram does not depend on any model\DIFdelbegin \DIFdel{and. 
Thus, the approach can}\DIFdelend \DIFaddbegin \DIFadd{. 
The approach can, thus, }\DIFaddend be applied to a variety of situations.

The Bandt-Pompe methodology and its variants have been used successfully in the analysis of many types of dynamics, receiving so far more than \DIFdelbegin \DIFdel{\num{1800} }\DIFdelend \DIFaddbegin \DIFadd{\num{2,160} }\DIFaddend citations, according to the Web of Science\DIFaddbegin \footnote{\DIFadd{Checked on 23 October 2021}}\DIFaddend .
...
The \DIFdelbegin \DIFdel{\num{1820} citations received by the seminal paper appeared in \num{684} journals }\DIFdelend \DIFaddbegin \DIFadd{seminal paper has received \num{2,160} citations from articles published in \num{780} venues }\DIFaddend indexed by the Web of Science.
\DIFdelbegin \DIFdel{These }\DIFdelend \DIFaddbegin \DIFadd{Among them, the }\DIFaddend journals belong to \num{127} categories, spanning from Multidisciplinary Physics (\SI{24}{\percent} of the publications) to Zoology (only one of the \DIFdelbegin \DIFdel{of }\DIFdelend citing articles).
There are \DIFdelbegin \DIFdel{\num{17} }\DIFdelend \DIFaddbegin \DIFadd{\num{22} }\DIFaddend citing articles from journals that belong to the Statistics \& Probability category.
Five of these articles appeared in \textit{Stochastic Environmental Research and Risk Assessment}, 
two in the \textit{Journal of Time Series Analysis} \DIFdelbegin \DIFdel{, 
and }\DIFdelend \DIFaddbegin \DIFadd{and in 
}\textit{\DIFadd{Theory and Applications of Time Series Analysis}}\DIFadd{.
Furthermore, }\DIFaddend each of the remaining ten appeared in a different journal.
...
With the knowledge of the expected variability of such points, \DIFdelbegin \DIFdel{according to the underlying dynamics, }\DIFdelend we can make \DIFdelbegin \DIFdel{hypothesis }\DIFdelend \DIFaddbegin \DIFadd{hypotheses }\DIFaddend tests for a wide variety of models \DIFaddbegin \DIFadd{according to the underlying dynamics}\DIFaddend .
...
\DIFdelbegin \DIFdel{In this proposal, the input is a sequence }\DIFdelend \DIFaddbegin \DIFadd{We used sequences }\DIFaddend of true random observations generated by a physical-based procedure \DIFdelbegin \DIFdel{. 
We obtain the confidence regions by performing an orthogonal projection of the data onto the space of }\DIFdelend \DIFaddbegin \DIFadd{to produce clouds of points. 
We then project these points onto their }\DIFaddend principal components, \DIFdelbegin \DIFdel{thus eliminating the }\DIFdelend \DIFaddbegin \DIFadd{find the regions in this representation, and transform them back to the Entropy-Complexity plane.
In this way, we circumvent the }\DIFaddend restrictions imposed by \DIFdelbegin \DIFdel{the bounded space of the Entropy-Complexity plane}\DIFdelend \DIFaddbegin \DIFadd{this 2D bounded manifold.
We also provide an algorithm that, using geometrical arguments, computes the empirical $p$-value of a sequence under the white noise null hypothesis}\DIFaddend .
...
\begin{itemize}
	...
	\item We verify the test's power contrasting \DIFaddbegin \DIFadd{(i)~}\DIFaddend correlated noise time series \DIFaddbegin \DIFadd{and (ii)~white noise series patched with a deterministic signal}\DIFaddend .
\end{itemize}
...
\DIFdelbegin \DIFdel{In our work , we consider }\DIFdelend \DIFaddbegin \DIFadd{Our work considers }\DIFaddend the ordinal patterns formed from true white noise sequences (TWNS) using the Bandt and Pompe symbolization.
...
It is zero when $h(j)=1$ for some $j$ (and, thus, all other bins are zero), and one when $h(j)=1/D!$ for every $j$ (the uniform probability \DIFdelbegin \DIFdel{distribution }\DIFdelend function).
...
They employed the Euclidean distance between $\bm h$ and the uniform probability \DIFdelbegin \DIFdel{distribution function.
}\DIFdelend \DIFaddbegin \DIFadd{function.
}\DIFaddend 

\DIFaddbegin \DIFadd{The Jensen-Shannon distance between $\bm h$ and the probability function $\bm u=(u(1), u(2), \dots, u(D!))$ stems as an adequate measure of such a distance:
}\begin{equation}
	\DIFadd{Q'(\bm{h}, \bm{u}) = \sum_{\ell=1}^{D!} \Big(h(\ell) \log\frac{h(\ell)}{u(\ell)} +
	u(\ell) \log\frac{u(\ell)}{h(\ell)}
	\Big).
	\label{eq:JensenShannon}
}\end{equation}
\DIFadd{This has been the preferred distance function since its proposal by \mbox{%DIFAUXCMD
\citet{Lamberti2004}}\hspace{0pt}%DIFAUXCMD
.
Most works consider the uniform distribution $\bm u = (1/D!, 1/D!, \dots, 1/D!)$ as the reference model.
}

\DIFadd{The quantity~}\eqref{eq:JensenShannon} \DIFadd{is also called ``disequilibrium.''
...
\DIFaddend With this, \DIFdelbegin \DIFdel{they }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{Lamberti2004} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend proposed $C=HQ$ as a measure of the Statistical Complexity of the underlying dynamics.
\DIFaddbegin \DIFadd{Notice that both $H$ and $Q$ are normalized quantities, so also is $C$. 
Moreover, $C$ is capable of discerning between different degrees of periodicity, among other relevant properties.
}

\DIFaddend A time series can then be mapped into a point in the $H\times C$ plane.
...
\DIFaddbegin \DIFadd{In particular, notice that the point associated with white noise is not at $(1,0)$, resulting from a finite rather than infinite length sequence.
}\DIFaddend 
...
Due to the infinitude of white noise sequences, although these sequences have the characteristic of presenting high \DIFdelbegin \DIFdel{values of entropy }\DIFdelend \DIFaddbegin \DIFadd{entropy values }\DIFaddend and low statistical complexity, their points will not necessarily be located in $(1, 0)$, but in a surrounding region.
Our study \DIFdelbegin \DIFdel{'s focus is to assess }\DIFdelend \DIFaddbegin \DIFadd{focuses on assessing }\DIFaddend pure randomness by analyzing the empirical distribution of the points produced by true random sequences of finite size and obtaining regions of confidence in the $H \times C$ plane.
...
It was found that \DIFdelbegin \DIFdel{the measure }\DIFdelend \DIFaddbegin \DIFadd{measures }\DIFaddend of statistical complexity \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend able to efficiently quantify the performance of pseudorandom number generators, expanding the possibilities of using Information Theory descriptors with the Bandt-Pompe symbolization \citep{larrondo2013statistical, gonzalez2005statistical}.
...
The authors \DIFdelbegin \DIFdel{make assessments about }\DIFdelend \DIFaddbegin \DIFadd{assess }\DIFaddend the randomness of \DIFdelbegin \DIFdel{sequence on }\DIFdelend \DIFaddbegin \DIFadd{sequences by }\DIFaddend ad hoc visual inspection of the point's location in the $H\times C$ plane.
...
Our goal is to find a \DIFdelbegin \DIFdel{latent space representative of }\DIFdelend \DIFaddbegin \DIFadd{space to represent }\DIFaddend the data without the \DIFdelbegin \DIFdel{restrictions of the }\DIFdelend $H\times C$ plane's boundaries \DIFaddbegin \DIFadd{restrictions}\DIFaddend .
...
\DIFdelbegin \DIFdel{Pseudorandom results can be achieved by software }\DIFdelend \DIFaddbegin \DIFadd{Software can produce pseudorandom sequeces }\DIFaddend in a deterministic way, but some applications need actual random numbers (despite the somewhat elusive nature of actual randomness).
...
The setup consists of an ordinary laser source to generate a local oscillator (LO), a half-wave plate, a polarizing beamsplitter (BPS), and two balanced detectors\DIFdelbegin \DIFdel{working together }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend adding or subtracting the photocurrents results in a quadrature measurement of the LO or vacuum state. 
...
This generator was implemented over a distributed setup with radios \DIFdelbegin \DIFdel{located }\DIFdelend at different geographical locations sending random bits to a cloud server that processes data and hosts the values.
The history of this service and other information can be found at \citet{RandomOrg}.

We used \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\SI{54e6}{4\byte} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\SI{54e6}{\byte} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend words from each physical generator, which approximately amounts \SI{200}{\mega\byte} of data.
...
We conducted an ablation study to identify the influence of the parameters $T$, $D$, and $\tau$ in \DIFdelbegin \DIFdel{the construction of }\DIFdelend \DIFaddbegin \DIFadd{constructing }\DIFaddend empirical confidence regions.
We verified that the results involving the time delay parameter variation did not show significant differences in repeated experiments; therefore, \DIFdelbegin \DIFdel{in the sequel, we did no }\DIFdelend \DIFaddbegin \DIFadd{we did not }\DIFaddend consider $\tau$ as a determining factor \DIFaddbegin \DIFadd{in the sequel}\DIFaddend .
...
We used the \texttt R platform \citep[][v.~4.0.3]{Rmanual} for data generation and analyses, and the \texttt{ggplot2} library \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{ggplot2Wickman} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{ggplot2Wickman} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend for generating the plots.

\DIFdelbegin %DIFDELCMD < \begin{comment}%DIFDELCMD < 
%DIFDELCMD < 	
%DIFDELCMD < 	\begin{figure}
%DIFDELCMD < 		\centering
%DIFDELCMD < 		\includegraphics[width=\linewidth]{Figures/Points-PDF.png}
%DIFDELCMD < 		\caption{White noise samples considered during the construction of the proposed confidence regions.}
%DIFDELCMD < 		\label{fig:white-noise}
...
The procedure operates on the principal components space and \DIFdelbegin \DIFdel{consists of measuring }\DIFdelend \DIFaddbegin \DIFadd{measures }\DIFaddend the closeness between the ``emblematic point'' and the observed point.
...
\DIFdelbegin %DIFDELCMD < \begin{comment}%DIFDELCMD < 
%DIFDELCMD < 	The empirical $p$-value of $\bm x'$ for the null hypothesis is given by the percentage of points outside the lowest confidence region obtained by the reference points to which $\bm x'$ belongs.
%DIFDELCMD < 	Therefore, it can be achieved by the following steps:
%DIFDELCMD < 	\begin{enumerate}
%DIFDELCMD < 		\item Compute $(h',c')$ the point in the $H\times C$ plane produced by $\bm x'$;
%DIFDELCMD < 		\item Compute $b_{T,D}(\bm x')$: the smallest box which contains $(h',c')$ using the $M$ reference points;
%DIFDELCMD < 		\item The empirical $p$-value of $\bm x'$ is obtained by the percentage of reference points outside $b_{T,D}(\bm x')$.
%DIFDELCMD < 	\end{enumerate}
%DIFDELCMD < 	Thus, we can verify that:
%DIFDELCMD < 	\begin{itemize}
%DIFDELCMD < 		\item the smallest $b_{T,D}(\bm x')$ is observed when $(h',c')$ ``coincides'' with the point produced by the emblematic sequence, in other words, point whose first principal component is the median of the $M$ reference points, and
%DIFDELCMD < 		\item the largest box is any box associated to $(h',c')$ ``outside'' the largest box produced by the $M$ reference points.
%DIFDELCMD < 	\end{itemize}
%DIFDELCMD < 	Assuming the critical value $\alpha = 0.05$, we obtain:
%DIFDELCMD < 	\begin{equation*}
%DIFDELCMD < 		p-\text{value} \geq \alpha, \text{ the null hypothesis should not be rejected}.
...
%DIFDELCMD < 		\caption{Coordinates in the $H\times C$ plane of the emblematic series and the points that define the confidence regions at \SI{90}{\percent}, \SI{95}{\percent}, \SI{99}{\percent}, and \SI{99.9}{\percent} for $D=3,4,5,6$ and $N=1000,50000$}\label{Tab:Points}
%DIFDELCMD < 		\centering
%DIFDELCMD < 		\begin{tabular}{rcllllllll}
%DIFDELCMD < 			\toprule
%DIFDELCMD < 			& & \multicolumn{8}{c}{$N$}\\ \cmidrule(lr){3-10}
%DIFDELCMD < 			& & \multicolumn{4}{c}{$1000$} & \multicolumn{4}{c}{$50000$}\\ \cmidrule(lr){3-6} \cmidrule(lr){7-10}
%DIFDELCMD < 			$D$ & Point & \SI{90}{\percent} & \SI{95}{\percent} & \SI{99}{\percent} & \SI{99.9}{\percent} & \SI{90}{\percent} & \SI{95}{\percent} & \SI{99}{\percent} & \SI{99.9}{\percent}\\ \cmidrule(lr){3-3} \cmidrule(lr){4-4} \cmidrule(lr){5-5} \cmidrule(lr){6-6} \cmidrule(lr){7-7} \cmidrule(lr){8-8} \cmidrule(lr){9-9} \cmidrule(lr){10-10}
%DIFDELCMD < 			$3$ & $\bm P'$ & \multicolumn{4}{c}{$h',c'$} & \multicolumn{4}{c}{$h',c'$}\\
%DIFDELCMD < 			& $\bm P_1$ & $(0.9973334, 0.0025601)$ & $(0.9967311, 0.0031343)$ & $(0.9953009, 0.0045054)$ & $(0.9931825, 0.0065387)$ & $(0.9999489, \num[scientific-notation=true]{5.06e-05})$ & $(0.9999384, \num[scientific-notation=true]{6.11e-05})$ & $(0.9999079, \num[scientific-notation=true]{9.11e-05})$ & $(0.9998625, 0.0001361)$\\
%DIFDELCMD < 			& $\bm P_2$ & $(0.9974047, 0.0026304)$ & $(0.9968219, 0.0032238)$ & $(0.9954349, 0.0046375)$ & $(0.9933704, 0.006724)$ & $(0.9999487, \num[scientific-notation=true]{5.04e-05})$ & $(0.9999382, \num[scientific-notation=true]{6.09e-05})$ & $(0.9999077, \num[scientific-notation=true]{9.09e-05})$ & $(0.9998622, 0.0001358)$\\
%DIFDELCMD < 			& $\bm P_3$ & $(0.9999497, 0)$ & $(0.9999398, 0)$ & $(0.9999203, 0)$ & $(0.9998925, 0)$ & $(0.9999998, \num[scientific-notation=true]{4e-07})$ & $(0.9999994, \num[scientific-notation=true]{9e-07})$ & $(0.9999982, \num[scientific-notation=true]{2e-06})$ & $(0.9999973, \num[scientific-notation=true]{3e-06})$\\
...
%DIFDELCMD < 			$4$ & $\bm P'$ & \multicolumn{4}{c}{$h',c'$} & \multicolumn{4}{c}{$h',c'$}\\
%DIFDELCMD < 			& $\bm P_1$ & $(0.994364, 0.0081246)$ & $(0.9937138, 0.0089796)$ & $(0.9922575, 0.0108947)$ & $(0.9902578, 0.0135243)$ & $(0.9999684, \num[scientific-notation=true]{3.98e-05})$ & $(0.9999725, \num[scientific-notation=true]{3.44e-05})$ & $(0.9999783, \num[scientific-notation=true]{2.68e-05})$ & $(0.9999833, \num[scientific-notation=true]{2.02e-05})$ \\
%DIFDELCMD < 			& $\bm P_2$ & $(0.9939234, 0.0075452)$ & $(0.9932534, 0.0083741)$ & $(0.9917308, 0.0102022)$ & $(0.9897312, 0.0128318)$ & $(0.9999696, \num[scientific-notation=true]{4.13e-05})$ & $(0.9999737, \num[scientific-notation=true]{3.6e-05})$ & $(0.9999795, \num[scientific-notation=true]{2.83e-05})$ & $(0.9999845, \num[scientific-notation=true]{2.18e-05})$ \\
%DIFDELCMD < 			& $\bm P_3$ & $(0.9994791, 0.0013982)$ & $(0.9991609, 0.0018166)$ & $(0.9987924, 0.0023012)$ & $(0.9985727, 0.0025901)$ & $(0.9998075, 0.0002508)$ & $(0.9998506, 0.0001942)$ & $(0.9998756, 0.0001615)$ & $(0.9998889, 0.000144)$ \\
%DIFDELCMD < 			& $\bm P_4$ & $(0.9990385, 0.0008188)$ & $(0.9987005, 0.0012111)$ & $(0.9982658, 0.0016087)$ & $(0.9980461, 0.0018976)$ & $(0.9998087, 0.0002524)$ & $(0.9998518, 0.0001958)$ & $(0.9998768, 0.000163)$ & $(0.9998901, 0.0001456)$ \\ \midrule
%DIFDELCMD < 			$5$ & $\bm P'$ & \multicolumn{4}{c}{$h',c'$} & \multicolumn{4}{c}{$h',c'$}\\
%DIFDELCMD < 			& $\bm P_1$ & $(0.9811818, 0.0321294)$ & $(0.9801289, 0.0340045)$ & $(0.977917, 0.0377295)$ & $(0.9753326, 0.0425299)$ & $(0.9998172, 0.0003232)$ & $(0.9998259, 0.0003075)$ & $(0.9998428, 0.0002774)$ & $(0.9998573, 0.0002517)$ \\
%DIFDELCMD < 			& $\bm P_2$ & $(0.9827429, 0.0350291)$ & $(0.9817117, 0.0369446)$ & $(0.9796031, 0.0408613)$ & $(0.9770187, 0.0456617)$ & $(0.9998194, 0.0003273)$ & $(0.9998282, 0.0003116)$ & $(0.999845, 0.0002814)$ & $(0.9998593, 0.0002553)$ \\
%DIFDELCMD < 			& $\bm P_3$ & $(0.9919707, 0.0120896)$ & $(0.9909376, 0.0139279)$ & $(0.9898161, 0.0156277)$ & $(0.9892599, 0.0166608)$ & $(0.9994812, 0.0009246)$ & $(0.9996371, 0.0006455)$ & $(0.9996703, 0.0005862)$ & $(0.9996884, 0.000554)$ \\
%DIFDELCMD < 			& $\bm P_4$ & $(0.9935319, 0.0149893)$ & $(0.9925204, 0.016868)$ & $(0.9915021, 0.0187595)$ & $(0.9909459, 0.0197926)$ & $(0.9994834, 0.0009286)$ & $(0.9996394, 0.0006495)$ & $(0.9996725, 0.0005901)$ & $(0.9996904, 0.0005576)$ \\ \midrule
%DIFDELCMD < 			$6$ & $\bm P'$ & \multicolumn{4}{c}{$h',c'$} & \multicolumn{4}{c}{$h',c'$}\\
%DIFDELCMD < 			& $\bm P_1$ & $(0.9121895, 0.2201993)$ & $(0.9105951, 0.2239294)$ & $(0.9105951, 0.2239294)$ & $(0.9077672, 0.2305874)$ & $(0.9990169, 0.002336)$ & $(0.9990368, 0.002288)$ & $(0.9990736, 0.0021997)$ & $(0.9991069, 0.0021197)$ \\
%DIFDELCMD < 			& $\bm P_2$ & $(0.9146048, 0.2260776)$ & $(0.9130413, 0.2298829)$ & $(0.9130413, 0.2298829)$ & $(0.9102595, 0.2366531)$ & $(0.9990249, 0.0023554)$ & $(0.9990449, 0.0023074)$ & $(0.9990817, 0.0022191)$ & $(0.999115, 0.0021392)$ \\
%DIFDELCMD < 			& $\bm P_3$ & $(0.9443868, 0.1418373)$ & $(0.9419202, 0.1476904)$ & $(0.9396577, 0.1531967)$ & $(0.9383611, 0.1561279)$ & $(0.9978983, 0.0050219)$ & $(0.998714, 0.0030633)$ & $(0.998765, 0.0029407)$ & $(0.9987884, 0.0028845)$ \\
%DIFDELCMD < 			& $\bm P_4$ & $(0.9468021, 0.1477156)$ & $(0.9443663, 0.1536439)$ & $(0.9421039, 0.1591502)$ & $(0.9408534, 0.1621937)$ & $(0.9979064, 0.0050413)$ & $(0.998722, 0.0030827)$ & $(0.9987731, 0.0029601)$ & $(0.9987965, 0.0029039)$ \\ \bottomrule
...
We also show the projection of the $H \times C$ plane boundaries in this space \DIFdelbegin \DIFdel{, as well }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend the median of each data set, the latter \DIFdelbegin \DIFdel{being }\DIFdelend represented as red dots.
...
They tend to concentrate close to \DIFdelbegin \DIFdel{the point that corresponds to }\DIFdelend $(1,0)$ in the $H\times C$ plane.
...
\begin{figure}[hbt]
	...
	\caption{New $100$ TRWNS and \DIFdelbeginFL \DIFdelFL{regions of }\DIFdelendFL confidence \DIFaddbeginFL \DIFaddFL{regions}\DIFaddendFL .}
	\label{fig:RNG}
\end{figure}
...
\DIFaddbegin \subsubsection{\DIFadd{$H_1$: Correlated Noise}}

\DIFaddend We assessed the power of the test by contrasting time series with different correlation \DIFdelbegin \DIFdel{structure }\DIFdelend \DIFaddbegin \DIFadd{structures }\DIFaddend (under the $f^{-k}$ model) in the $H \times C$ plane.
...
As the correlation increases with $k$, the randomness decreases, causing a drop in the entropy; the series \DIFdelbegin \DIFdel{become }\DIFdelend \DIFaddbegin \DIFadd{becomes }\DIFaddend progressively more predictable.
...
\DIFdelbegin \subsection{\DIFdel{Revisiting the White Noise Hypothesis in the Literature}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{$H_1$: Patch of a Deterministic Function}}\label{Sec:PatchDeterministic}
\DIFaddend 

\DIFaddbegin \DIFadd{Consider the times series of length $T$ comprised of $T-[c100]$ white noise observations $x_1,x_2,\dots, x_{T-[c100]}$ followed by the signal $y_{T-[c100]+1}, y_{T-[c100]+2}, \dots, y_T$, in which $0\leq c \ll 1$ is the percentage of contamination.
We will verify the behavior of our test for the case $T=1000$, and $y_t$ an increasing function on $t$.
This kind of contamination appears, for instance, when imputing missing data by linear interpolation~\mbox{%DIFAUXCMD
\citep{ImputationforConsecutiveMissingValuesinNonStationaryTimeSeriesData}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFadd{Fig.~\ref{Fig:PointsPatchedIncreasingFunction} shows the effect of such contamination on the white noise time series whose point in the $H\times C$ plane appears in black.
The Figure also shows the confidence region at \mbox{%DIFAUXCMD
\SI{99}{\percent}}\hspace{0pt}%DIFAUXCMD
.
}
...
\begin{figure}[hbt]
	...
	\caption{\DIFaddFL{Effect of patching an increasing function at different levels to a white noise sequence.}}\label{Fig:PointsPatchedIncreasingFunction}
\end{figure}
%DIF > %% The code that produces this figure starts at line 221, file "container_function.R"

\DIFadd{At all levels of contamination, the original point is displaced to a region with smaller entropy and larger complexity.
When the percentage of contamination is below approximately \mbox{%DIFAUXCMD
\SI{2}{\percent}}\hspace{0pt}%DIFAUXCMD
, the test will not reject the null hypothesis at the \mbox{%DIFAUXCMD
\SI{1}{\percent} }\hspace{0pt}%DIFAUXCMD
level of significance; in fact, the $p$-value is of around \mbox{%DIFAUXCMD
\SI{1.5}{\percent} }\hspace{0pt}%DIFAUXCMD
for $c=0,0.01,0.02$.
The $p$-value falls to orders of $10^{-6}$ when the contamination is of \mbox{%DIFAUXCMD
\SI{3}{\percent} }\hspace{0pt}%DIFAUXCMD
and above.
}

\DIFadd{It is worth noticing that a test based solely on the entropy would have less power: it would only start rejecting when the contamination attains approximately \mbox{%DIFAUXCMD
\SI{6}{\percent} }\hspace{0pt}%DIFAUXCMD
of the signal, or more.
The complexity component enables the test to reject the null hypothesis at small contamination percentages.
}

\subsection{\DIFadd{Revisiting the White Noise Hypothesis in the literature}}

\DIFaddend In this section, we compare the performance of our test with that of 
...
We grouped our results in those that rejected (R) the null hypothesis and those that did not reject it (NR)\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, given the $p$-value obtained by our test. 
...
\begin{table}
	\caption{Results of the sequences generated by the main PRNGs in the literature. 
		...
		\DIFaddbeginFL \DIFaddFL{The ``Decision'' of rejecting (R) or not (NR) is at the $\SI{5}{\percent}$ significance level.}\DIFaddendFL }
	...
	\begin{tabular}{cccc}
		...
		\multicolumn{1}{c}{$D$} & 
		$p$-value &
		\DIFdelbeginFL \DIFdelFL{HC-PCA}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Decision}\DIFaddendFL \\
		\cmidrule(lr){1-1}
		\cmidrule(lr){2-2}
		\cmidrule(lr){3-3}
		\cmidrule(lr){4-4}
...
(1)~building \DIFdelbegin \DIFdel{nonparametric }\DIFdelend \DIFaddbegin \DIFadd{non-parametric }\DIFaddend empirical confidence regions in the principal components space and mapping these boxes back to the $H\times C$ plane.
...
Our test has \DIFdelbegin \DIFdel{a }\DIFdelend good size, mostly with long TWNRS.
...
Although our work focuses on the study of short sequences, we \DIFdelbegin \DIFdel{were able to capture }\DIFdelend \DIFaddbegin \DIFadd{captured }\DIFaddend the random behavior of well-known pseudorandom number generators already analyzed in the literature. 
With this, we verified the adequacy of our technique as it is capable of detecting correlation structures\DIFaddbegin \DIFadd{.
}

\DIFadd{Although the Bandt and Pompe approach does not replace classical techniques, it is a valuable and widespread approach with an immediate graphical appeal.
Making statistical inference in the Entropy-Complexity plane is challenging, but its potential impact is huge}\DIFaddend .
